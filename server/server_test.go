...
// Metrics are an aggregated view across listeners.
func TestCheckMetricsServer(t *testing.T) {
	...
	var nrOfRequests int64 = 0
	//Given a test server which sends a response of a given size and reads everythign
	s := server.NewBasicServer(tlsPort, "localhost", tlsCert, tlsKey, CreateTestHandler(t, int64(responseSize)), testPort)
	...
	//WHEN we do a HTTP request with a given size
	...
	var finishedRequests float64 = 0
	for i := 0; i < 3; i++ { // First time fetching metrics make sure they have actually propagated.
		_, finishedRequests = getCounterMetric(t, metricsPort, "http_requests_finished_total")
		if checkCloseEnough(nrOfRequests, finishedRequests, 0.001) == nil {
			break
		}
		time.Sleep(time.Millisecond * time.Duration(100))
	}

	//THEN after waiting we must be at 1
	assertCloseEnough(t, nrOfRequests, finishedRequests, 0.001)

	_, requestSizeBytes := getCounterMetric(t, metricsPort, "http_request_size_bytes")
	//THEN the returned size should be close to the actual request size
	assertCloseEnough(t, nrOfRequests*sendSize, requestSizeBytes, 0.001)

	//THEN we have the same expectations for response size
	_, responseSizeBytes := getCounterMetric(t, metricsPort, "http_response_size_bytes")
	assertCloseEnough(t, nrOfRequests*responseSize, responseSizeBytes, 0.001)

	//WHEN we do a HTTPS request with a given size
	r, err = http.NewRequest("POST", fmt.Sprintf("https://localhost:%d", tlsPort), testutils.NewNonDeterministicLimitedRandReadSeeker(sendSize))
	if err != nil {
		t.Error("Could not create request")
		t.FailNow()
	}
	...
	res, err = client.Do(r)
	if err != nil {
		t.Error("Could not perform HTTPS request", "error", err)
		t.FailNow()
	}
	nrOfRequests += 1
	...
	//THEN after waiting we must be at 2
	assertCloseEnough(t, nrOfRequests, finishedRequests, 0.001)
...
func checkCloseEnough(targetValue int64, actualValue float64, percentage float64) error {
	...
	if actualValue < lowerBound {
		return fmt.Errorf("Actual value too small expected %d, got %f", targetValue, actualValue)
	}
	if actualValue > upperBound {
		return fmt.Errorf("Actual value too big expected %d, got %f", targetValue, actualValue)
	}
	return nil
}
...
func assertCloseEnough(t testing.TB, targetValue int64, actualValue float64, percentage float64) {
	err := checkCloseEnough(targetValue, actualValue, percentage)
	if err != nil {
		t.Error(err)
	}
}
