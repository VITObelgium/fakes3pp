...
func NewPolicyManagerForLocalPolicies(policyPath string) (*PolicyManager, error) {
	pm := NewPolicyManager(
...
func (r *LocalPolicyRetriever) retrievePolicyStr(arn string) (string, error) {
	...
	c, err := utils.ReadFileFull(filePath)
	if err != nil {
		return "", err
	}
	return string(c), err
...
func restartWatching(watcher *fsnotify.Watcher, fileName string) {
	...
	if _, err := os.Stat(fileName); errors.Is(err, os.ErrNotExist) {
		slog.Info("Cannot restart watch since file is gone", "filename", fileName)
	} else {
